import {gpioHardwareOperation, spiHardwareOperation, constructNowEvent, postHardwareOperation} from './api'
...
var app_scores = [Number(home_score_ele.innerHTML), Number(away_score_ele.innerHTML)]
...
async function seven_segment_display(numbers) {
    var hardware_opers = [];
    const home_score = numbers[0];
    const away_score = numbers[1];

    if (home_score >= MAX_NUMBER || home_score < 0 ||
        away_score >= MAX_NUMBER || away_score < 0) {
    ...
    if (home_ser_pin === away_ser_pin) {
        ...
        spiHardwareOperation(hardware_opers, 0, home_ser_pin, undefined, 
            home_srclk_pin, undefined, 500, 0, 0, 0, ...spi_transmit_data);
        ...
        spiHardwareOperation(hardware_opers, 0, away_ser_pin, undefined, 
            away_srclk_pin, undefined, 500, 0, 0, 0, ...spi_transmit_data);
...
document.getElementById("startScore").addEventListener("click", async function(event) {
...
document.getElementById("homeSegmentPinSelect").addEventListener("change", function(event) {
...
document.getElementById("awaySegmentPinSelect").addEventListener("change", function(event) {
...
document.getElementById("resetScore").addEventListener("click", async function() {
    await resetScores();
});
document.getElementById("addOneHome").addEventListener("click", async function() {
    await changeScore("home", 1);
});
document.getElementById("addTwoHome").addEventListener("click", async function() {
    await changeScore("home", 2);
});
document.getElementById("addThreeHome").addEventListener("click", async function() {
    await changeScore("home", 3);
});
document.getElementById("minusOneHome").addEventListener("click", async function() {
    await changeScore("home", -1);
});
document.getElementById("addOneAway").addEventListener("click", async function() {
    await changeScore("away", 1);
});
document.getElementById("addTwoAway").addEventListener("click", async function() {
    await changeScore("away", 2);
});
document.getElementById("addThreeAway").addEventListener("click", async function() {
    await changeScore("away", 3);
});
document.getElementById("minusOneAway").addEventListener("click", async function() {
    await changeScore("away", -1);
});